function [c,ww] = smoothPaul(varargin)
%SMOOTH  Smooth data.
%
% 180209 https://www.mathworks.com/matlabcentral/mlc-downloads/downloads/submissions/28367/versions/1/previews/LAMB_toolbox_0.1/smooth.m/index.html
%
...
%   Copyright 2018 Paully 
%   $Revision: 1.17 $  $Date: 2002/05/29 18:43:31 $

if nargin < 1
  error('SMOOTHPaul needs at least one argument.');
end
...
if nargin==1 | ( nargin > 1 & (length(varargin{2})==1 | isstr(varargin{2})) )
  % smoothPaul(Y) | smoothPaul(Y,span,...) | smoothPaul(Y,method,...)
...
if nargin == 1+is_x | isstr(varargin{2+is_x})
  % smoothPaul(Y), smoothPaul(X,Y) | smoothPaul(X,Y,method,..), smoothPaul(Y,method)
  is_span = 0;
else
  % smoothPaul(...,SPAN,...)
  is_span = 1;
...
if nargin >= 2+is_x+is_span 
  % smoothPaul(...,Y,method,...) | smoothPaul(...,Y,span,method,...)
...
if isempty(span), span = 5; end % smoothPaul(Y,[],method)
...
switch method
 ...
 otherwise
  error('SMOOTHPaul: Unrecognized method.');
end
