...
if size(x,2)<size(cv,2)
	x=x(:,1)*ones(size(cv(1,:)));
end

if na>2,
	if length(arg3)==size(cv,2)
		shift=conj(arg3(:)');
	else
		shift=arg3;
	end
...
if length(shift)==1
	shift=shift*[0:1:size(cv,2)-1];	
end

if size(shift,1)~=size(cv,1)
	shift=ones(size(cv(:,1)))*shift;
end
if na>1
	nstick=arg2;
	dl=floor(length(cv)/nstick);
	index=[1:dl:nstick*dl]';
	[cv,shift,x]=vindex(cv,shift,x,index,1);
end
...
weave=zeros(size(cv,1)*3,size(cv,2));
index=[1+(0:length(cv)-1)*3]';
weave(index,:)=x;
weave(index+1,:)=real(cv)./factor+sqrt(-1)*imag(cv)+x;
weave(index+2,:)=nan; 
...
hold on
cv=real(cv)./factor+sqrt(-1)*imag(cv);
len=max(max(imag(cv)))/10;
ax=axis;
top=ax(4)*sqrt(-1);
right=ax(2);
