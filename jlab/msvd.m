...
%
%   The input matrix W is an J x K x M matrix where 
%
%          J is the number of transforms (frequencies or scales)
%          K is the number of eigentransforms (tapers or wavelets)
%          M is the number of dataset components
%
%   The output is
%
%	       D  --  J x MIN(K,M) matrix of singular values
%	      U1  --  J x M matrix of first left singular vectors
...
%   right singular vectors use the convention of Lilly and Olhede (2006) 
%   and not of Park et al. (1987), who use a transposed version of W. 
%
%   W may optionally be of size N x J x M x K, in which case the
%   output arguments are all three-dimensional matrices with sizes
%
%	       D  --  N x J x MIN(K,M) matrix of singular values
%	      U1  --  N x J x M matrix of first left singular vectors
%	      V1  --  N x J x K matrix of first right singular vectors
%
...
%   matrix, which is of size J (for 3-D W) or N x J (for 4-D W).  
%
...
%   (C) 1993--2006 J.M. Lilly --- type 'help jlab_license' for details        
