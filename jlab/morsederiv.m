...
%
%   DCELL=MORSEDERIV(N,GAMMA,BETA,OMEGA) returns the first N 
%   normalized frequency derivatives of the lowest-order generalized 
%   Morse wavelet specified by parameters GAMMA and BETA, evaluated at 
...
% 
%   N must be a scalar. The other input parameters must either be 
%   matrices of the same size, or some may be matrices and the others
%   scalars.    
%
%   DCELL=MORSEDERIV(N,GAMMA,BETA) evaluates the derivative at the
%   peak frequency by default.
%
...
kcell{1}=0*om;
for n=1:nmax;
    kcell{n+1}=om.^n.*morsekappa(ga,be,om,n);
end
psi=cum2mom(kcell);
psi=psi(2:end);
...
kappan=frac(be,om.^n).*(-1).^(n-1).*factorial(n-1)-om.^(ga-n).*prodn;


function[psi1,psi2,psi3,psi4]=morsederiv_peak(ga,be,om);
...
[psi1b,psi2b,psi3b,psi4b]=morsederiv_peak(ga,be,om);
