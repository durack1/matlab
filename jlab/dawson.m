...
%
%   Y = DAWSON(X) is Dawson's integral for each element of X.
%
%   X must be real.
%   
...
%   see for example Lilly and Olhede (2008b).
%  
%   See also ERF, ERFC, DAWSONDERIV.  
%
%   'dawson --t' runs a test.
%
%   Usage: x=dawson(x);
%   __________________________________________________________________
%   Written 2004 P.J. Acklam, redistributed with permission, thank you 
%   This is part of JLAB --- type 'help jlab' for more information
%   (C) 2004, 2007  P.J. Acklam and J.M. Lilly 
...
t=[-150:dt:150]';
g=exp(-t.^2);
psi1=frac(2,sqrt(pi))*dawson(t);
psi2=imag(hilbert(g));
err=vsum(abs(psi1-psi2).^2,1)./vsum(abs(psi1).^2,1);
%plot(t,[psi1 psi2])

reporttest('DAWSON equals Hilbert transform of Gaussian',err<1e-3)
