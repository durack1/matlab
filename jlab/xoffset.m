...
%   contour plots, etc.). XOFFSET also allow multiple axes to be
...
%   column of the X-data by an amount N from the previous column.  N
%   must be a number, not a variable whose value is a number.
%
%   XOFFSET(N) or XOFFSET N, where N is an *imaginary* number, offsets
%   each column of the X-data by an amount imag(N)*DX from the
%   previous column, where DX is the X-axis length of the original
%   (i.e. unoffset) plot.
%
%   XOFFSET(M,N) or XOFFSET M N, applies the same offsets to each of
%   M groups of lines, i.e. for complex-valued data Z use UVPLOT(Z) 
%   followed by XOFFSET 2 N.
%
%   XOFFSET with no arguments returns the data to its original
%   (unoffset) orientation [as do XOFFSET(0) and XOFFSET(0i)].
%   
%   XOFFSET LOCK and XOFFSET UNLOCK lock and unlock all Y-data in the
%   current figure. When LOCK is on, calls to XOFFSET are applied to
%   each subplot individually.
%
%   See also YOFFSET, LINERING, LINESTYLE
...
if length(delta)==1
   for i=1:length(xx)
       xx{i}=xx{i}+delta*(mod(i-1,round(length(xx)/N)));
   end
