...
% $Revision 1.34.6.4 $  $Date: 2005/03/23 20:23:54 $
...
errstring = [];
if nans
    errstring = 'Ignoring NaNs in data.';
end
if infs
    inferr = 'Ignoring Infs in data.';
...
if cplx
    cplxerr = 'Using only the real component of complex data.';
    if isempty(errstring)
        errstring = cplxerr;
    else
        errstring = sprintf('%s\n%s', errstring, cplxerr);
...
function update(ignore1,ignore2,ds)
...
function newname(ignore1,ignore2)
...
function cleanup(ignore1,ignore2,dataset)
