...
%   $Revision: 1.9.2.3 $  $Date: 1999/09/24 21:02:37 

if ~isequal(size(x),size(y))
   error('curvefit:excludedata:xyDifferentSizes', ...
         'XDATA and YDATA must have the same size.');
end

s=size(x);
if min(s)~=1
   error('curvefit:excludedata:xyNotVectors', ...
         'XDATA and YDATA must both be vectors.');
end
...
case 0
   error('curvefit:excludedata:invalidMethod', ...
         '"%s" is not a valid method.', method);
