...
%
%   The following coefficient names are not allowed in the expression 
%   string EXPR: i, j, pi, inf, nan, eps.
%
...
%                    there are two or more). Note excluded names above.
...
%   $Revision: 1.25.2.9 $  $Date: 2006/11/11 22:39:34 $
...
if isequal(class(varargin{1}),'fittype') && nargin==1  % if fittype object, just return
    obj = varargin{1};
    return
elseif isa(varargin{1},'fittype') && nargin==1  % elseif child of fittype object, return fittype
    structobj = struct(varargin{1});
    obj = structobj.fittype;
    return
...
if isequal('custom',obj.category)
    ...
    if obj.linear
        linearvars = {};
        for i=1:length(obj.Adefn)
            linearvars = vertcat( linearvars, symvar( obj.Adefn{i} ) );
        end
...
if isequal('custom',category(obj))
    try
        ...
        if islinear(obj) % Also eval to get coefficient matrix
            getcoeffmatrix(obj,probparams{:},xtemp);
        end
        ...
        temp = feval(obj,vals{:},probparams{:},xtemp); %#ok
...
if mod(n,2)
    error('curvefit:fittype:invalidParam', ...
          'FITTYPE requires parameter name/value pairs.');
end
for i = 1:floor(n/2)
    param = arglist{1};
    value = arglist{2};
