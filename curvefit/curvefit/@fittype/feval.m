...
%   $Revision: 1.16.2.4 $  $Date: 2005/03/07 17:26:03 $
...
if FITTYPE_OBJ_.feval
    ...
    if (isempty(FITTYPE_OBJ_.expr))
        ...
        try
           ...
           [varargout{1:max(1,nargout)}] = eval(FITTYPE_OBJ_.expr);
        catch
           error('curvefit:fittype:feval:expressionError',...
                 'Error in fittype expression ==> %s\n??? %s',...
                 FITTYPE_OBJ_.expr,lasterr);
        end                
    end
end
