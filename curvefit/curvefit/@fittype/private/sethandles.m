...
%   $Revision: 1.26.2.10 $  $Date: 2006/10/02 16:31:40 $
...
if separable
    [b,y,wts,ignore,x] = deal(varargin{:});
    sqrtwts = sqrt(wts);
    ws = warning('off', 'all');
...
separable = isequal(varargin{end-1},'separable');
if separable
    [b,d,y,wts,ignore,x] = deal(varargin{:});
    sqrtwts = sqrt(wts);
...
%---------------------------------------------------------
function [deriv1,deriv2]  = power2der(a,b,ignore,x)
...
if separable
  [w,y,wts,ignore,n,x] = deal(varargin{:});
  sqrtwts = sqrt(wts);
...
% Apply fast fourier transfrom to the data points
fy = fft(y-mean(y));  % subtract mean to get rid of constant effect

% Find the peak frequency
[maxf,maxloc] = max(fy(1:floor(length(x)/2)));
...
for j=1:n
    ...
    [maxf,maxloc] = max(fy(1:floor(lengthx/2)));
    oldpeaks(end+1) = maxloc;
...
if lengthx < 2
    error('curvefit:fittype:ssethandles:twoDataPointsRequired', ...
          'Fit requires at least two data points.');
end
