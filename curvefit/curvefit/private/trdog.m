...
%   $Revision: 1.4.2.3 $  $Date: 2005/03/07 17:27:00 $
...
if isempty(Z)
   if isempty(v1)
      ...
      case 'jacobprecon'
         [R,permR] = feval(pcmtx,H,pcoptions,DM,DG,varargin{:});
      otherwise
         error('curvefit:trdog:invalidPreconflagString', ...
...
arg = (abs(s) > 0);
if isnan(s)
   error('curvefit:trdog:TrustRegionStepContainsNans', ...
         'Trust region step contains NaNs.')
end
...
if n > 1 
   ...
   if norm(ssssave) < .9*delta
      ...
      arg = (abs(ns) > 0); 
      if isnan(ns)
         error('curvefit:trdog:reflectedTrustRegionStepContainsNans', ...
               'Reflected trust region step contains NaNs.')
      end
      % No truncation if s is zero length
      if isempty(find(arg))   
         alpha = 1;
   ...
   if isnan(sg)
      % No truncation if s is zero length
        error('curvefit:trdog:GradientStepContainsNans', ...
              'Gradient region step contains NaNs.')
   end
   if  isempty(find(arg))   
      alpha = 1;
   else
...
numer = -(b + sign(b)*sqrt(b^2-4*a*c));
warnstate = warning('off', 'all'); % Avoid divide by zero warnings
r1 = numer/(2*a);
r2 = c/(a*r1);
warning(warnstate);
...
tau = min(1,tau);
if tau <= 0, 
   error('curvefit:trdog:squareRootError', ...
