...
%
%   CFSMTHSPL is based on the function CSAPS in the Spline Toolbox.
%
%   See also CSAPS in the Spline Toolbox.

%   Copyright 2000-2006 The MathWorks, Inc.
%   $Revision: 1.13.2.5 $ $Date: 2006/12/30 08:00:22 $
...
n = length(xi);
if n < 2
	error('curvefit:cfsmthspl:notEnoughPoints', ...
          'Smoothing spline requires at least two data points.')
end

[yd,yn] = size(yi);
if n ~= yn
	error('curvefit:cfsmthspl:NumSitesValuesDoNotMatch', ...
          'Number of data sites and data values must match.')
end
yi = yi.';
dd = ones(1,yd);
...
if isempty(w)
	...
	if ~isempty(ignorep)
		...
		n = length(xi);
		if n < 2
			error('curvefit:cfsmthspl:notEnoughPositiveWeights', ...
                 ['There should be at least two data points with' ...
					' positive weights.'])
		end
	end
end
...
if n == 2 % the smoothing spline is the straight line interpolant
   ...
   elseif (p < 0) || (p > 1)
      error('curvefit:cfsmthspl:invalidParam', ...
            'Smoothing parameter must be between 0 and 1.');
   end
   M = 6*(1-p)*QtWQ+p*R;
   u = M \ diff(divdif);
