...
%   $Revision: 1.9.2.4 $  $Date: 2005/03/07 17:26:51 $
...
if neqcstr>0
    ...
    [Q,R,A,B,CIND,X,Z,ignore,how,...
            ACTSET,ACTIND,ACTCNT,aix,eqix,neqcstr,ncstr,remove,exitflag]= ...
        eqnsolv(A,B,eqix,neqcstr,ncstr,numberOfVariables,X, ...
        aix,how,exitflag);   
...
if mc>eps
    ...
    defaultopt = options;
    [XS,lambdaS,ignore1,ignore2] = lsqsub([],[zeros(numberOfVariables,1);1],A2,[B;1e-5], ...
...
if ~isempty(depInd)
    ...
    if any( bdepInd ) % Not consistent
        ...
        [ignore1,ignore2,Eat]=qr(ACTSET');        
        [i,ignore] = find(Eat); % Eat permutes the columns of A' (rows of A)
        remove = i(depInd);
        A(eqix(remove),:)=[];
        B(eqix(remove))=[];
