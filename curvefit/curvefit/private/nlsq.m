function [x,CostFunction,JAC,EXITFLAG,OUTPUT,msg] = nlsq(funfcn,x,verbosity,options,defaultopt,CostFunction,JAC,YDATA,ignore,varargin)
...
%   $Revision: 1.12.2.3 $  $Date: 2005/03/07 17:26:52 $
...
% OPT_STOP is used for canceling fits
...
if ischar(maxFunEvals)
   if isequal(lower(maxFunEvals),'100*numberofvariables')
      ...
      error('curvefit:nlsq:maxFunEvalsMustBeInt', ...
...
[OLDX,ignore]=lsinit(XOUT,CostFunction,verbosity,levMarq);
PCNT = 0;
EstSum=0.5;
...
while status~=1  && ~isequal(OPT_STOP, 1)
   iter = iter + 1;
   % Work Out Gradients
...
%         SD=-(GRAD*GRAD'+(norm(GRAD)+1)*(eye(numberOfVariables,numberOfVariables)))\(GRAD*CostFunction);
      ...
      elseif NewF<FIRSTF
         ...
         if ~levMarq
            if currstepsize>1e-8 && cond(JAC)<1e8
               ...
               if verbosity > 0
                  warning('curvefit:nlsq:poorGradientConditioning', ...
                          ['Conditioning of Gradient Poor - ',...
                           'Switching To Levenberg-Marquardt method.']);
               end
               how='CHG2LM';
               levMarq=1;
   ...
   elseif iter > maxIter
      ...
      if PCNT~=0
         ...
         while PCNT > 0 && ~isequal(OPT_STOP, 1) && numFunEvals <= maxFunEvals
            x(:) = XOUT; 
            CostFunction = feval(funfcn{3},x,varargin{:});
            if ~isempty(YDATA)
               CostFunction = CostFunction - YDATA;
         ...
         if OPT_STOP
             error('curvefit:nlsq:fittingCancelled', ...
   ...
   case 'fun_then_grad'
      ...
      numGradEvals=numGradEvals+1;
   otherwise
      error('curvefit:nlsq:undefinedCalltype', ...
            'Undefined calltype in CFLSQCURVEFIT');
   end
   numFunEvals=numFunEvals+1;
