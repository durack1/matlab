% COLOURPLOTF  x-y plot of filled symbols, whose colour is determined by z.
% INPUT:  xx    x coords
%         yy    y coords
...
% Paul Durack 3/10/08 - Updated code to include filling
% Paul Durack 7/1/09  - Updated code to include MarkerEdgeColor as black
%
% USAGE: colourplotf(xx,yy,zz,symb,cpmin,cprng,msz,blur)

function colourplotf(xx,yy,zz,symb,cpmin,cprng,msz,blur)

if nargin == 0
    disp('colourplotf(xx,yy,zz,{symb,cpmin,cprng,msz})');
    return
end
if all(isnan(zz(:)))
    disp('COLOURPLOTF: no non-nan values')
    return
end
...
if strcmp(symb(1),'-') | strcmp(symb,':')
    ...
    if cprng>0
        ...
        for ii=1:lcm-1
            ...
            plot(xx(idx(kk)),yy(idx(kk)),symb,'Color',cm(ii,:),'MarkerSize',msz,'MarkerFaceColor',cm(ii,:),'MarkerEdgeColor','k');
            idx(kk) = [];
        end
        plot(xx(idx),yy(idx),symb,'Color',cm(lcm,:),'MarkerSize',msz,'MarkerFaceColor',cm(ii,:),'MarkerEdgeColor','k');
    else
        ...
        plot(xx(idx),yy(idx),symb,'Color',cm(1,:),'MarkerSize',msz,'MarkerFaceColor',cm(ii,:),'MarkerEdgeColor','k');
    end
end
