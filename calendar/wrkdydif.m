...
%   WRKDYDIF determines the number of working days between two dates (inclusive)
%   given a number of holidays.
%
%   NumberDays = wrkdydif(Date1, Date2, NumberHolidays)
%
%   Inputs: 
%            Date1 - Nx1 or 1xN vector containing the date string or serial date
%                    number for the start date.
%
%            Date2 - Nx1 or 1xN vector containing the date string or serial date
%                    number for the end date.
%
%   NumberHolidays - Nx1 or 1xN vector containing values for the number of
%                    Holidays between the two dates.
%
%   Outputs: 
%       NumberDays - Nx1 or 1xN vector containing the number of days between
...
%   $Revision: 1.10.2.8 $   $Date: 2011/02/28 01:21:21 $
...
if nargin < 1
   error(message('finance:wrkdydif:missingInputs'))
end
...
if any(NumberHolidays < 0)
   error(message('finance:wrkdydif:invalidNumberOfHolidays'));
end
...
sizes = [size(Date1); size(Date2); size(NumberHolidays)];
if any(sizes(:,1)~=sizes(1,1)) || any(sizes(:,2)~=sizes(2,2))
   error(message('finance:wrkdydif:invalidInputDims'))
end
