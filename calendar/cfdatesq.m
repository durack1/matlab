...
% $Revision: 1.3.2.12 $   $Date: 2010/10/08 16:42:29 $ 
...
[CouponRate    , Settle             , Maturity  , Period         , ...
 ...
 LastCouponDate, periodsBeforeSettle, periodsAfterMaturity] = instargbondmod(0 , varargin{:}); %#ok
...
% be a user-callable function, I have allowed INSTARGBOND
   ...
   switch nargin 
      case 9
        ...
        if any(periodsBeforeSettle < 0)
           error(message('finance:cfdatesq:negPerBefSet'));
        end
      ...
      case 10
        ...
        if any(periodsAfterMaturity < 0)
           error(message('finance:cfdatesq:negPerAftMat'));
        end
...
Rule = zeros(size(EndMonthRule));
Rule(EndMonthRule == 0) = 4; 
